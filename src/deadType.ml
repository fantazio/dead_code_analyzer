...
let nb_args ~keep typ =
  let rec loop n = function
    ...
    | Tarrow (Labelled _, _, typ, _) when keep = `Lbl ->
        loop (n + 1) (get_desc typ)
    | Tarrow (Optional _, _, typ, _) when keep = `Opt ->
        loop (n + 1) (get_desc typ)
...
let rec _TO_STRING_ typ =
  begin [@warning "-11"]
    ...
    | Ttuple l -> (
        ...
        | e :: l ->
            List.fold_left
              (fun prev typ -> prev ^ " * " ^ _TO_STRING_ typ)
              (_TO_STRING_ e) l
        | [] -> "*"
      )
...
let collect_export path u stock t =
  let stock = if stock == DeadCommon.decs then decs else stock in

  let save id loc =
    if t.type_manifest = None then export path u stock id loc;
    let path =
      String.concat "." @@ List.rev_map (fun id -> Ident.name id) (id :: path)
    in
...
let tstr typ =
  ...
  | Ttype_variant l ->
      List.iter
        (fun { Typedtree.cd_name; cd_loc; _ } ->
          assoc cd_name cd_loc.Location.loc_start _variant
        )
        l
  | _ -> ()

let report () =
  report_basic decs "UNUSED CONSTRUCTORS/RECORD FIELDS" !DeadFlag.typ
