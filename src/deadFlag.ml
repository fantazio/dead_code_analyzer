...
type threshold =
  { exceptions : int; percentage : float; optional : [ `Percent | `Both ] }
...
let opta =
  ref
    { print = false;
      call_sites = false;
      threshold = { exceptions = 0; percentage = 1.; optional = `Percent }
    }
let optn =
  ref
    { print = false;
...
let update_opt opt s =
  let threshold s =
    ...
    if len > 5 && String.sub s 0 5 = "both:" then (
      ...
      try
        opt :=
          { !opt with
            threshold = { !opt.threshold with exceptions = int_of_string thr }
          };
        opt :=
          { !opt with
            threshold = { !opt.threshold with percentage = float_of_string pct }
          }
  ...
  | s ->
      ...
      else if !opt.threshold.percentage > 1. || !opt.threshold.percentage < 0.
      then raise (Arg.Bad "-Ox: percentage must be >= 0.0 and <= 1.0")

type style = { opt_arg : bool; unit_pat : bool; seq : bool; binding : bool }
let style =
  ref { opt_arg = false; unit_pat = false; seq = false; binding = false }

let update_style s =
  let rec aux = function
    | (b, "opt") :: l ->
        style := { !style with opt_arg = b };
        aux l
    | (b, "unit") :: l ->
        style := { !style with unit_pat = b };
        aux l
    | (b, "seq") :: l ->
        style := { !style with seq = b };
        aux l
    | (b, "bind") :: l ->
        style := { !style with binding = b };
        aux l
    | (b, "all") :: l ->
        style := { unit_pat = b; opt_arg = b; seq = b; binding = b };
        aux l
    | (_, "") :: l -> aux l
    ...
    try
      let rec split acc pos len =
        if str.[pos] <> '+' && str.[pos] <> '-' then
          ...
          let acc =
            (str.[pos] = '+', String.trim (String.sub str (pos + 1) len)) :: acc
          in
          if pos > 0 then split acc (pos - 1) 0 else acc
      in
      split [] (String.length str - 1) 0
    with _ ->
      raise
        (Arg.Bad "options' arguments must start with a delimiter (`+' or `-')")
  in
  aux (list_of_opt s)

type basic = { print : bool; call_sites : bool; threshold : int }
let exported : basic ref =
  ref ({ print = true; call_sites = false; threshold = 0 } : basic)
...
let update_basic opt (flag : basic ref) = function
  ...
  | s ->
      ...
      let threshold =
        let len = String.length s in
        if len > 6 && String.sub s 0 6 = "calls:" then (
      ...
      if threshold < 0 then
        raise
          (Arg.Bad
             (opt ^ ": integer should be >= 0; Got " ^ string_of_int threshold)
          )
...
let normalize_path s =
  ...
  let rec norm_path = function
    | [] -> []
    | x :: (y :: _ as yss) when x = y && x = Filename.current_dir_name ->
        norm_path yss
    | x :: xss ->
        if x = Filename.current_dir_name then
          norm_path xss (* strip leading ./ *)
        else
