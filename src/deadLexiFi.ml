...
let dyn_rec =
  ref [] (* Record names used for dynamic typing and locations of those uses *)
...
let () =
  (DeadLexiFi.sig_value :=
     fun value ->
       ...
       in
       let add = function
         | { attr_name = { txt = "mlfi.value_approx"; _ };
  ...
  (DeadLexiFi.tstr_type :=
     fun typ ctype ->
       ...
       in
       if is_user_defined ctype then hashtbl_add_to_list field_link path ctype
  );
    ...
    fun decs ->
      ...
      let rec process (p, typ, call_site) =
        ...
        | Tconstr (path, ts, _) ->
            ...
            let name =
              if String.contains name '.' then name else p ^ "." ^ name
            in
            let met = ref [] in
            let rec proc name =
              if not (List.mem name !met) then (
                met := name :: !met;
                name
                :: List.fold_left
                     (fun acc name -> acc @ proc name)
                     []
            ...
            in
            List.iter
              (fun typ -> hashtbl_add_to_list dyn_used typ call_site)
              (proc name);
