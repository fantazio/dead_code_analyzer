...
let repr_loc loc =
  ...
  let rec loop loc =
    if Hashtbl.mem equals loc then
      ...
      if not (Hashtbl.mem met next) then (
        Hashtbl.add met loc ();
        loop next
      )
      else loc
...
let meth_tbl tbl loc =
  if not (Hashtbl.mem tbl loc) then Hashtbl.add tbl loc (Hashtbl.create 16);
...
let get_loc path =
  let path =
    let exported_path =
      ...
      |> Seq.find (fun (_, exported_path) ->
             is_sub_path ~sep:'.' path exported_path
         )
    in
    match exported_path with
    | Some (_, exported_path) -> exported_path
    | None -> try Hashtbl.find defined path with Not_found -> path
  in
...
let add_equal loc1 loc2 =
  ...
  if loc1 <> loc2 && not (is_ghost loc1 || is_ghost loc2) then (
    Hashtbl.replace equals loc1 loc2;
    if loc1 = !last_class then last_class := loc2;
    let meths =
      let res = hashtbl_find_list content loc2 in
      if res = [] then hashtbl_find_list content loc1 else res
    in
    List.iter
      (fun (_, meth) ->
        hashtbl_merge_unique_list (meth_ref loc2) meth (meth_ref loc1) meth;
        hashtbl_merge_unique_list (meth_self_ref loc2) meth (meth_self_ref loc1)
          meth
      )
      meths;
...
let rec sign = function
  | Cty_signature sg -> sg
  | Cty_arrow (_, _, t) | Cty_constr (_, _, t) -> sign t

let rec treat_fields action typ =
  match get_deep_desc typ with
  | Tobject (t, _) | Tarrow (_, _, t, _) -> treat_fields action t
  | Tfield (s, k, _, t) ->
      if field_kind_repr k <> Fabsent && s.[0] > 'Z' then action s;
...
let collect_export path u stock ~obj ~cltyp loc =
  ...
  ( match List.rev_map (fun id -> Ident.name id) path with
  | h :: t
  ...
  let save id =
    if not (Sys.file_exists (Filename.chop_extension !current_src ^ ".csml"))
    then export ~sep:"#" path u stock (Ident.create_persistent id) loc
  in
...
let tstr
    ( { ci_expr;
        ci_decl = { cty_loc = loc; _ };
        ci_id_name = { txt = name; _ };
        _
      },
      _
    ) =
  ...
  if not (Hashtbl.mem defined short) then Hashtbl.add defined short path
  else
    let loc =
    ...
    else add_path path !last_class;

    let rec make_dep ci_expr =
      match ci_expr.cl_desc with
      | Tcl_ident (path, _, _) ->
          let loc = get_loc (Path.name path) in
          if loc != Lexing.dummy_pos then add_equal !last_class loc
      | Tcl_fun (_, _, _, ci_expr, _) | Tcl_constraint (ci_expr, _, _, _, _) ->
          make_dep ci_expr
      | _ -> ()
    in
    make_dep ci_expr
...
let class_structure cl_struct =
  let rec add_aliases pat =
    ...
    | (Tpat_alias _ | Tpat_var _) when not pat.pat_loc.Location.loc_ghost ->
...
let class_field f =
  let rec locate cl_exp =
    match cl_exp.cl_desc with
    | Tcl_ident (path, _, _) ->
        let path = Path.name path in
        if Hashtbl.mem defined path then Hashtbl.find defined path else path
    | Tcl_fun (_, _, _, cl_exp, _)
    | Tcl_apply (cl_exp, _)
    | Tcl_let (_, _, _, cl_exp)
    | Tcl_constraint (cl_exp, _, _, _, _) ->
        locate cl_exp
  ...
  | Tcf_method ({ txt; _ }, _, _) -> update_overr true txt
  | _ -> ()
  );
  hashtbl_replace_list content (repr_loc !last_class)
    (hashtbl_find_list content !last_class)

let arg typ args =
  let rec collect_type_related_references typ arg =
    match get_deep_desc typ with
    | Tarrow (_, _, typ, _) -> collect_type_related_references typ arg
    | Tobject _ -> (
        match arg with
        | (_, None) -> ()
        | (_, Some exp) ->
...
let coerce expr typ =
  let loc = locate expr in
  let use meth =
    hashtbl_add_unique_to_list (meth_ref loc) meth
      expr.exp_loc.Location.loc_start
  in
